...
- **Run `--mode gate` in CI.** Deterministic checks, flow replay and all regression rules run with no model at all. Zero tokens. Many teams run gate on every PR and review nightly.
